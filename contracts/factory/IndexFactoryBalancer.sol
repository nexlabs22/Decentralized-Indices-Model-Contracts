    ...
    function reIndexAndReweight() public onlyOwner {
        ...
        for (uint256 i; i < totalCurrentList; i++) {
            ...
            if (tokenAddress != address(weth)) {
                bool success =
                    vault.withdrawFunds(tokenAddress, address(this), IERC20(tokenAddress).balanceOf(address(vault)));
        ...
        for (uint256 i; i < totalOracleList; i++) {
            ...
            if (tokenAddress != address(weth)) {
                bool success = vault.withdrawFunds(address(weth), address(this), wethBalance);
                require(success, "Vault withdrawal failed");
                uint256 outputAmount = swap(
                    address(weth),
                    tokenAddress,
