...
const toWei = (i : string) => ethers.parseEther(i)
const toEther = (i : any) => ethers.formatEther(i)
...
describe("Uniswap v3 test", function () {
  ...
  async function deployOneYearLockFixture() {
    ...
    const weth:any = await ethers.getContractAt("IWETH", WETH9)
    const dai:any = await ethers.getContractAt("@openzeppelin/contracts/token/ERC20/IERC20.sol:IERC20", DAI)
    const qouterContract:any = await ethers.getContractAt("IQuoter", QOUTE);
    //deploy swap contract
    const SwapExamples:any = await ethers.getContractFactory("SwapExamples");
    const swapExamples:any = await SwapExamples.deploy();
