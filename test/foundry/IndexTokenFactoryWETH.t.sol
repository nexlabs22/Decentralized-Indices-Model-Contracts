...
contract IndexTokenFactoryWETHTest is Test, ContractDeployer {
    ...
    function testOracleList() public {
        ...
        assertEq(factoryStorage.oracleList(4), address(weth));
        // token current list
        ...
        assertEq(factoryStorage.currentList(4), address(weth));
        // token shares
        ...
        assertEq(factoryStorage.tokenOracleMarketShare(address(weth)), 20e18);
        ...
        assertEq(factoryStorage.tokenSwapFee(address(weth)), 3000);
        ...
        (address[] memory path4, uint24[] memory fees4) = factoryStorage.getFromETHPathData(address(weth));
        assertEq(path4[0], address(weth));
        assertEq(path4[1], address(weth));
        ...
        (address[] memory path9, uint24[] memory fees9) = factoryStorage.getToETHPathData(address(weth));
        assertEq(path9[0], address(weth));
