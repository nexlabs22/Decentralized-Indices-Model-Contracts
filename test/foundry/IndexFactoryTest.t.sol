...
contract IndexFactoryTest is Test, IndexFactory {
    ...
    function test_MutationCoverage_Lines275_283() public {
        updateOracleList();

        address user = address(0xAAA);

        deal(address(usdt), user, 100_000 ether);
        deal(user, 100 ether);

        vm.startPrank(user);
        usdt.approve(address(indexFactory), type(uint256).max);

        indexFactory.issuanceIndexTokens(address(usdt), 2000e18, 3000);

        uint256 mintedIndexTokens = indexToken.balanceOf(user);

        indexFactory.redemption(mintedIndexTokens, address(weth), 3000);
...

        {
            vm.startPrank(user);
            vm.expectRevert();
            indexFactory.redemption(1e18, address(weth), 3000);
    ...
    function updateOracleList() public {
        ...
        // link.transfer(address(Fstorage), 1e17);
